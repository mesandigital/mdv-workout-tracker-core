...
export function useUpdateSetRows(sessionId: number) {
  ...
  const mutation = useMutation({
    ...
    mutationFn: async (params: { setId?: number; reps?: number; weight?: number; exerciseLogId?: number; plannedReps?: number; dropSets?: SetRow['dropSets']; plannedDurationSeconds?: number | null; durationSeconds?: number | null; completed?: number }) => {
      if (params.setId !== undefined) {
        // Update set_log (set row)
        return WorkoutSessionApi.updateSetLog(params.setId, params.plannedReps ?? 0, params.weight ?? 0, params.reps, params.dropSets, params.plannedDurationSeconds, params.durationSeconds, params.completed);
      } else if (params.exerciseLogId !== undefined) {
        // Update exercise_log (weight/plannedReps for the exercise in this session)
        return WorkoutSessionApi.updateExerciseLog(params.exerciseLogId, params.weight, params.plannedReps);
      } else {
        throw new Error('Either setId or exerciseLogId must be provided');
      }
    },
    onMutate: async (variables) => {
      await queryClient.cancelQueries({ queryKey: ['workoutSession', sessionId] });
        ...
        if (variables.setId !== undefined) {
            ...
            sets: ex.sets.map((s: SetRow) =>
              s.id === variables.setId
                ? {
                  ...
                  plannedDurationSeconds: variables.plannedDurationSeconds ?? s.plannedDurationSeconds,
          ...
          newExercises = old.exercises.map((ex: any) =>
            ex.exerciseLogId === variables.exerciseLogId
              ? {
                  ...ex,
                  weight: variables.weight !== undefined ? variables.weight : ex.weight,
                  plannedReps: variables.plannedReps !== undefined ? variables.plannedReps : ex.plannedReps,
                }
              : ex
          );
        } else {
