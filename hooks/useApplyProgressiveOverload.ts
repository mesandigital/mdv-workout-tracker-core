...
import { ProgressiveOverloadApi, ProgressiveOverloadRecommendation, WorkoutSessionApi } from '../sessions';
...
export function useApplyProgressiveOverload() {
  ...
  const mutation = useMutation({
    mutationFn: async ({ sessionId, progressiveOverload, perExerciseOverload, recommendations }: ApplyPOInput) => {
      return ProgressiveOverloadApi.applyProgressiveOverload(
        sessionId,
        typeof progressiveOverload === 'number' ? progressiveOverload : 2.5,
        perExerciseOverload,
        typeof progressiveOverload === 'object' ? progressiveOverload : undefined,
        recommendations,
      );
    },
