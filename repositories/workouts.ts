...
const normalizeSets = (exercise: WorkoutTemplateExerciseInput): WorkoutTemplateExerciseSetInput[] => {
...
const normalizeBlockType = (exercise: WorkoutTemplateExerciseInput): WorkoutBlockType | null => {
  const blockType = exercise.block_type
    || (exercise.group_type === 'circuit' || exercise.group_type === 'superset' ? exercise.group_type : null);
  if (blockType === 'straight_sets' || blockType === 'circuit' || blockType === 'superset' || blockType === 'giant_set' || blockType === 'interval') {
    return blockType;
  }
  return null;
};
...
const isRoundBasedBlock = (blockType: WorkoutBlockType | null) => (
  blockType === 'circuit' || blockType === 'superset' || blockType === 'giant_set'
);
...
async function ensureWorkoutBlock(
  ...
  const sourceGroupId = exercise.group_id || exercise.superset_id || exercise.block_id || index + 1;
  ...
  const blockId = await insert('workout_blocks', {
    ...
    type: blockType,
    name: exercise.block_name || (blockType === 'circuit' ? 'Circuit' : blockType === 'superset' ? 'Superset' : null),
    rounds: isRoundBasedBlock(blockType)
      ? Math.max(1, Math.round(exercise.block_rounds || exercise.default_sets || 1))
      : null,
...
async function insertWorkoutExercise(
  ...
  const groupType = exercise.group_type || (exercise.superset_id ? 'superset' : null);
  const groupId = exercise.group_id || exercise.superset_id || null;
  const blockId = await ensureWorkoutBlock(workoutId, exercise, index, blockIds);
  const blockType = normalizeBlockType(exercise);
...
export async function createWorkoutTemplate(input: WorkoutTemplateInput) {
  ...
  for (let index = 0; index < input.exercises.length; index += 1) {
    await insertWorkoutExercise(workoutId, input.exercises[index], index, blockIds);
  }

  return workoutId;
}

export async function updateWorkoutTemplate(id: number, input: WorkoutTemplateInput) {
  await updateWhere('workouts', {
    name: input.name,
    type: input.type || null,
    section: input.section || null,
    description: input.description || null,
...
export async function getWorkoutTemplate(id: number): Promise<WorkoutTemplate | null> {
...
export async function listWorkoutTemplates() {
  return selectRaw('SELECT * FROM workouts WHERE archived = 0 ORDER BY created_at DESC');
}

export async function archiveWorkoutTemplate(id: number) {
  await execute('UPDATE workouts SET archived = 1, updated_at = ? WHERE id = ?', [new Date().toISOString(), id]);
}
