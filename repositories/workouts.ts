...
async function insertWorkoutExercise(
  ...
  const workoutExerciseId = await insert('workout_exercises', {
    ...
    per_side: normalizedExercise.per_side ? 1 : 0,
...
export async function getWorkoutTemplate(
  ...
  const exercises = await selectRaw<any>(
    ...
    SELECT
      ...
      e.training_style as training_style,
      e.progression_group as progression_group,
      e.progression_level as progression_level,
  ...
  const hydratedExercises = await Promise.all(
    ...
    `,
      ...
      return {
        ...
        classification: exercise.exercise_category,
        category: exercise.category,
        movement: exercise.movement,
        difficulty: exercise.difficulty,
        trainingStyle: exercise.training_style,
        progressionGroup: exercise.progression_group,
