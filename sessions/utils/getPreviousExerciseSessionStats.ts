...
import { selectRawOne, selectRaw } from "../../../db-adapter";

// Utility: get stats for an exercise from the previous session (PR, last session completion, etc)
export async function getExerciseSessionStats(exerciseId: number, currentSessionId: number, exerciseName: string) {
    // Find last session (excluding current)
    const lastSessionRow = await selectRawOne<{ id: number; finished_at: string; workout_name: string | null }>(
        `
    ...
    if (!lastSessionRow) return { lastSessionDate: null, lastSessionId: null, lastSessionStats: null, isNewPR: false };
    ...
    const lastSets = rawLastSets.map((set) => {
        let dropSets = [];
        try {
            const parsed = typeof set.drop_sets === 'string' ? JSON.parse(set.drop_sets) : set.drop_sets;
    ...
    const allSetsCompleted = lastSets.length > 0 && lastSets.every(s => s.completed);

    // Calculate total reps completed and if all reps were completed (all sets have reps >= planned_reps)
    const totalRepsCompleted = lastSets.reduce((sum, s) => sum + (s.reps ?? 0), 0);
    const allRepsCompleted = lastSets.length > 0 && lastSets.every(s => typeof s.reps === 'number' && typeof s.planned_reps === 'number' && s.reps >= s.planned_reps);

    // Find best set (weight * reps) for previous session
    const lastBestSet = lastSets.length > 0
        ? lastSets.reduce((best, s) => ((s.weight ?? 0) * (s.reps ?? 0)) > ((best.weight ?? 0) * (best.reps ?? 0)) ? s : best, lastSets[0])
        : null;

    // Find best rep (highest reps in a set) for previous session
    const lastBestRep = lastSets.length > 0
        ? lastSets.reduce((best, s) => (s.reps ?? 0) > (best.reps ?? 0) ? s : best, lastSets[0])
        : null;

    // Calculate total volume for previous session
    const lastTotalVolume = lastSets.reduce((sum, s) => sum + (s.weight ?? 0) * (s.reps ?? 0), 0);
    ...
    const currentBestSet = currentSets.length > 0
        ? currentSets.reduce((best, s) => ((s.weight ?? 0) * (s.reps ?? 0)) > ((best.weight ?? 0) * (best.reps ?? 0)) ? s : best, currentSets[0])
        : null;
    const newBestSet = currentBestSet && lastBestSet
        ? ((currentBestSet.weight ?? 0) * (currentBestSet.reps ?? 0)) > ((lastBestSet.weight ?? 0) * (lastBestSet.reps ?? 0))
        : false;
    ...
    // OTHHERS
    // Gap info
    const gapInfo = lastSessionDate ? getExerciseGapInfo({
        lastSessionDate,
        today: new Date(),
        exerciseName
    }) : null;
    ...
    const sessions = await selectRaw<{ date: string; sessionId: string;  weight: number; reps: number }>(
        `
    ...
    const sessionsById: Record<string, { weight: number; reps: number; sessionId: string; date: string }[]> = {};
    sessions.forEach(s => {
        if (!sessionsById[s.sessionId]) {
            sessionsById[s.sessionId] = [];
        }
        sessionsById[s.sessionId].push({ weight: s.weight, reps: s.reps, sessionId: s.sessionId, date: s.date });
    });
    // convert to array of { date, sets }
    const recent = Object.entries(sessionsById).map(([_sessionId, sets]) => ({ date: sets[0].date, sets }));
    ...
    if (gapInfo) {
        (gapInfo as any).transledMessage = `It has been ${gapInfo.days} days since you trained ${exerciseName}.`;
