...
export type SqlExecutor = <T = any>(sql: string, params?: SqlValue[]) => Promise<T[]>;
...
export async function selectRawOne<T = any>(sql: string, params: SqlValue[] = []) {
...
export async function insert(table: string, data: Record<string, SqlValue>) {
  ...
  const row = await selectRawOne<{ id: number }>('SELECT last_insert_rowid() as id');
  if (!row) throw new Error(`Could not read inserted id for ${table}`);
  return row.id;
}

export async function updateWhere(table: string, data: Record<string, SqlValue>, where: string, params: SqlValue[]) {
  const keys = Object.keys(data).filter((key) => data[key] !== undefined);
  if (!keys.length) return;
  const setters = keys.map((key) => `${key} = ?`).join(', ');
  await execute(`UPDATE ${table} SET ${setters} WHERE ${where}`, [...keys.map((key) => data[key]), ...params]);
}

export async function removeWhere(table: string, where: string, params: SqlValue[]) {
...
export async function filterToExistingColumns<T extends Record<string, any>>(table: string, row: T) {
